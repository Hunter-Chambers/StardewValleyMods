    ...
    public class FishingMod : Mod
    {
        ...
        *********/
        ...
        private void OnMenuChanged(object? sender, MenuChangedEventArgs e)
        {
            ...
            {
                this.Bobber.Value = new SBobberBar(menu, this.Helper.Reflection);
            }
        }
            ...
            if (e.IsOneSecond && (this.Config.InfiniteBait || this.Config.InfiniteTackle))
            {
                if (Game1.player.CurrentTool is FishingRod rod)
                {
                    ...
                    if (this.Config.InfiniteTackle)
                    {
                        ...
                        if (rod.attachments?.Length > 2 && rod.attachments[2] is not null)
                            rod.attachments[2].uses.Value = 0;
                    }
                }
            }

            if (this.Config.AutoHookFish && Game1.player.CurrentTool is FishingRod fishingRod)
            {
                FishingHelper.AutoHook(fishingRod, !this.Config.DisableVibrations);
            }
